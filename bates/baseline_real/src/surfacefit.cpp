...
namespace qe{
    ...
    ostream& operator<<(ostream& os, const BatesSurfaceFit& fit) {
        os << fixed << setprecision(6);
    
        os << "Bates Surface Fit\n";
        os << "-------------------\n";
        os << "v0       : " << fit.v0 << "\n";
        ...
        os << "xi       : " << fit.xi << "\n";
        os << "rho      : " << fit.rho << "\n";
        os << "JumpIntensityQ      : " << fit.JumpIntensityQ << "\n";
        os << "JumpMeanQ      : " << fit.JumpMeanQ << "\n";
        os << "JumpVolatility      : " << fit.JumpVolatility << "\n";
        os << "\nFit Diagnostics\n";
        os << "----------------\n";
    ...
    ostream& operator<<(ostream& os, const BatesMultiSurfaceFit& fit)
    {
        os << "===== Bates Multi-Surface Fit =====\n";
    ...
    BatesSurfaceFit calibrateBatesQVolGrid(CallGrid& grid,BatesSurfaceFit& InitialGuess,const Date& today, const DayCounter& dc,const Calendar& cal){
        ...
        Real JumpIntensityQ_guess = InitialGuess.JumpIntensityQ;
        Real JumpMeanQ_guess = InitialGuess.JumpMeanQ;
        Real JumpVolatility_guess = InitialGuess.JumpVolatility;
        ...
        auto process = ext::make_shared<BatesProcess>(
            rTS, qTS, spot,
            v0_guess, kappaQ_guess, thetaQ_guess, xi_guess, rho_guess, JumpIntensityQ_guess, JumpMeanQ_guess, JumpVolatility_guess
        );
        ...
        for (int i = 0; i < grid.C.size();i++){
            ...
            // [TODO] - apparently bates version does not exist
        ...
        Simplex opt(0.05);
        // [TODO] - adjust end criteria according to Bates
        EndCriteria ec(
            100, 20,   // max iterations, stationary iterations
            1e-4, 1e-4, 1e-4
        );
        // model is an object (BatesModel inherits from CalibrateModel)
        ...
        out.rho    = params[3];
        out.v0     = params[4];
        out.JumpIntensityQ = params[5];
        out.JumpMeanQ = params[6];
        out.JumpVolatility = params[7];
    ...
    Real MultiSurfaceSSE(CallGrid& grid,BatesMultiSurfaceFit& phi,Real v0_guess,const DayCounter& dc,const Calendar& cal){
        ...
        Real JumpIntensityQ_guess = phi.JumpIntensityQ;
        Real JumpMeanQ_guess = phi.JumpMeanQ;
        Real JumpVolatility_guess = phi.JumpVolatility;
    ...
    BatesMultiSurfaceFit MultiSurfaceRandomSearch(const vector<CallGrid>& surfaces,const DayCounter& dc,const Calendar& cal,
        ...
        uniform_real_distribution<> dist_JumpIntensityQ(phi_init.JumpIntensityQ*(1-eps),phi_init.JumpIntensityQ*(1+eps));
        uniform_real_distribution<> dist_JumpMeanQ(phi_init.JumpMeanQ*(1-eps),phi_init.JumpMeanQ*(1+eps));
        uniform_real_distribution<> dist_JumpVolatility(phi_init.JumpVolatility*(1-eps),phi_init.JumpVolatility*(1+eps));
        
        BatesMultiSurfaceFit bestInitialFit;
        bestInitialFit.TotalSSE = QL_MAX_REAL;
        for (int i = 0; i < n_tries;i++){
            //cout<<"Try-"<< (i+1)<<endl;
            BatesMultiSurfaceFit candidate;
            candidate.kappaQ = dist_kappaQ(gen);
            candidate.thetaQ = dist_thetaQ(gen);
            candidate.xi = dist_xi(gen);
            candidate.rho = dist_rho(gen);
            candidate.JumpIntensityQ = dist_JumpIntensityQ(gen);
            candidate.JumpMeanQ = dist_JumpMeanQ(gen);
            candidate.JumpVolatility = dist_JumpVolatility(gen);
    ...
    QuantLib::Real value(const QuantLib::Array& x) const override {
        ...
        BatesMultiSurfaceFit phi;
        phi.kappaQ = std::exp(a);
        phi.thetaQ = std::exp(b);
        phi.xi     = std::exp(c);
        phi.rho    = std::tanh(d); 
        // [TODO] - check if these indexes are correct
        phi.JumpIntensityQ = std::exp(x[5]);
        phi.JumpMeanQ = x[6];
        phi.JumpVolatility = std::exp(x[7]);
...
BatesMultiSurfaceFit nedlerMeadMultiSurface(const vector<CallGrid>& surfaces,const DayCounter& dc,const Calendar& cal,const BatesMultiSurfaceFit& phi0){
    QuantLib::Array x0(8);
    ...
    // [TODO] - check if these indexes are correct
    x0[5] = std::log(std::max(Real(1e-12), phi0.JumpIntensityQ));
    x0[6] = phi0.JumpMeanQ;
    x0[7] = std::log(std::max(Real(1e-12), phi0.JumpVolatility));
    ...
    // [TODO] - check if these end criterias and solver conditions are fine
    ...
    out.JumpIntensityQ = exp(xstar[5]);
    out.JumpMeanQ = xstar[6];
    out.JumpVolatility = exp(xstar[7]);
