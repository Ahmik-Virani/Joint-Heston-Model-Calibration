...
private:
    ...
    vector<double> create_guess(){
        ...
        vector<double> true_params = data.get_true_params();

        thread_local mt19937 local_rng{random_device{}()};
        for(int i = 0 ; i < 7 ; i++){
            uniform_real_distribution<double> distr(0.5 * true_params[i], 1.5 * true_params[i]);
    ...
    void initialization(int particle_index, int time_index){
        ...
        surfaceCalibration this_particle(data.get_K(), data.get_T(), data.get_grid(time_index), guess, data.get_todays_date(time_index), data.get_S(time_index), data.get_r(), data.get_q());
...
public:
    ...
    smoothStateCalibration(generateData &passed_data, int n=10, int n_samples=50, bool to_use_ESS=true, int p=8){
        // Get the true data
        N=n;
        no_of_threads=p;
    ...
    void _update_particle(int t, int i, vector<double> &cur_weights, set<int> &reinitialized_indexes, set<int> &surviving_indexes, const double &observed_log_return, ofstream &log_file){
        ...
        double true_call_price = data.get_C(t);
        double computed_call_price = 0.0;

        const vector<double> qParams = P_to_Q(particles[i]);

        QuantLib::Date evalDate = data.get_todays_date(t);
        double tau = data.get_time_to_maturity(t);

        if (tau > 0.0) {
            computed_call_price = qe::hestonCallPrice(
                data.get_S(t),                     // spot
                data.get_strike(),                 // fixed strike
                tau,                               // maturity in years (scalar)
                data.get_r(),                      // r
                data.get_q(),                      // q
                max(1e-8, particles[i][6]),        // v0
                max(1e-8, qParams[2]),             // kappaQ
                max(1e-8, qParams[3]),             // thetaQ
                max(1e-8, particles[i][1]),        // xi
                clamp(particles[i][4], -0.999, 0.999),  // rho
                evalDate                                // anchor pricing at this day
            );
        }

        double pentaly = exp(-abs(true_call_price - computed_call_price));
        weights[i] *= pentaly;
        cur_weights[i] *= pentaly;
    ...
    void main(){
        ...
        for(int t = 1 ; t <= time_steps ; t++){
            ...
            if(reinitialized_indexes.size() != 0){
                ...
                for(int i : reinitialized_indexes){
                    ...
                    double penalty_for_i = exp(-abs(true_call_price - computed_call_price));
        ...
        // Finally let us store the actual S and v path in a csv file
        ofstream S_path("S_path.csv");
        ofstream v_path("v_path.csv");
        ...
        ofstream true_params("true_params.csv");
        auto params_true = data.get_true_params();
        int no_of_params = params_true.size();
        for(int i = 0 ; i < no_of_params ; i++){
            true_params << params_true[i];
            if(i != no_of_params - 1){
                true_params << ',';
            }
        }
