...
public:
    ...
    getRealData(){
        cout<<"I am here"<<endl;
        // get the stock path
        ifstream s_inp("./Data/S_path.csv");
        if (!s_inp.is_open()) {
        ...
        ifstream r_inp("./Data/r_path.csv");
        if (!r_inp.is_open()) {
        ...
        while(getline(r_inp, line)){
            ...
            // [TODO] - comment if only r required
            // date = date.substr(3);
            rates[date] = rate;
        }
        ...
        ifstream C_inp("./Data/C_grid.csv");
        if (!C_inp.is_open()) {
        ...
        while(getline(C_inp, line)){
            ...
            double maturityYears = dayCounter.yearFraction(this_date, this_expiry);
