...
class surfaceCalibrationMultiStartLM {
private:
    ...
    vector<double> residuals(const vector<double>& unconstrained) const {
        const vector<double> qParams = P_to_Q(make_original(unconstrained));
        vector<double> residual(grid_.size(), 0.0);
        if (!isValidQ(qParams)) {
            fill(residual.begin(), residual.end(), 1e4);
            return residual;
        }

        for (size_t i = 0; i < grid_.size(); ++i) {
            const auto& row = grid_[i];
            const double model = qe::hestonCallPrice(
                spot_, row[0], row[1], r_, q_, qParams[6], qParams[2], qParams[3], qParams[1], qParams[4], today_);
            residual[i] = row[2] - model;
        }
        return residual;
    }

    vector<double> runSingleLM(const vector<double>& initial, double& bestLoss) {
        struct Cost : public QuantLib::CostFunction {
            const surfaceCalibrationMultiStartLM& self;
            explicit Cost(const surfaceCalibrationMultiStartLM& s) : self(s) {}
            ...
            QuantLib::Array values(const QuantLib::Array& x) const override {
                ...
                vector<double> r = self.residuals(u);
                QuantLib::Array y(r.size());
        ...
        QuantLib::LevenbergMarquardt lm(1e-8, 1e-8, 1e-8);
    ...
    void runMultiStartLMCalibration() {
        ...
        for (size_t restart = 0; restart < restarts; ++restart) {
            ...
            vector<double> candidate = runSingleLM(start, thisLoss);
...
public:
    surfaceCalibrationMultiStartLM(vector<vector<double>> grid, vector<double> guesses, QuantLib::Date today, double spot, double r, double q) {
